-- Keep the official QR assignment history readable by the active COO agent
-- without depending on the Vercel service-role identity.
begin;
  ...
  from (
    ...
    where audit.action = 'INITIAL_ASSIGNMENT'
      and (
        ...
        or exists (
          select 1
          from public.agents viewer
          where viewer.id = auth.uid()
            and viewer.actif is true
            and upper(btrim(viewer.role)) = 'AGENT'
            and upper(btrim(viewer.agence)) = 'COTONOU'
        )
      )
