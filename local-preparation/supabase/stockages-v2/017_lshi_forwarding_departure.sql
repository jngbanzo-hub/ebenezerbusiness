...
begin
  ...
  if to_regprocedure('public.confirm_klz_forwarding_departure(text,text,numeric,text,numeric,numeric,text,date,uuid,text,uuid)') is null
    ...
    or not exists (
      select 1 from information_schema.columns
      where table_schema='public' and table_name='stockage_forwardings'
        and column_name='cash_event_id' and is_nullable='YES'
    )
    or not exists (
    ...
    or not exists (
      select 1 from pg_constraint
      where conrelid='public.stockage_forwardings'::regclass
        and conname='stockage_forwardings_status_check'
        and position('IN_TRANSIT' in pg_get_constraintdef(oid,true))>0
    )
    or not exists (
      select 1 from pg_constraint
      where conrelid='public.stockage_forwarding_orchestrations'::regclass
        and conname='stockage_forwarding_orchestrations_state_check'
        and position('IN_TRANSIT' in pg_get_constraintdef(oid,true))>0
    )
    or not exists (
      select 1 from pg_constraint
      where conrelid='public.stockage_forwarding_events'::regclass
        and conname='stockage_forwarding_events_event_type_check'
        and position('FORWARDING_DEPARTED' in pg_get_constraintdef(oid,true))>0
    )
  then raise exception 'MIGRATION_016_REQUIRED'; end if;
end $$;

create function public.confirm_storage_forwarding_departure(
