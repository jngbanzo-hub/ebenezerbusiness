...
function enregistrerPaiementAgent(
  ...
  if (
    ...
    return {
      succes: false,
      code: "PAYMENT_REQUEST_ID_INVALIDE",
      message:
  ...
  try {
    ...
    if (paymentRequestId) {
      const classeurDoublons =
        SpreadsheetApp.getActiveSpreadsheet();

      for (
        let indexFeuille = 0;
        indexFeuille <
          CONFIG_PAIEMENTS.feuilles.length;
        indexFeuille++
      ) {
        const feuilleDoublons =
          classeurDoublons.getSheetByName(
            CONFIG_PAIEMENTS.feuilles[
              indexFeuille
            ]
          );
        ...
        if (
          !feuilleDoublons ||
          feuilleDoublons.getLastRow() < 2
        ) {
          continue;
        }

        const paymentRequestIdsExistants =
          feuilleDoublons
            .getRange(
              2,
              16,
              feuilleDoublons.getLastRow() - 1,
              1
            )
            .getDisplayValues();

        const paiementDejaEnregistre =
          paymentRequestIdsExistants.some(
            (ligne) =>
              String(ligne[0] || "")
                .trim()
                .toLowerCase() ===
    ...
    feuillePaiement
      .getRange(
        ...
        1,
        1,
        valeursPaiement.length
      )
      .setValues([valeursPaiement]);
...
if (
  ...
  var modePaiement = body.modePaiement
    .trim()
    .toUpperCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "");
  ...
  if (body.paymentRequestId !== undefined) {
  ...
  // Seuls ces dix champs sont transmis à la fonction métier.
  return {
...
function filtrerReponseColisPourAgentPaiementsAgentsApi_(resultatMetier, destinationCode) {
  ...
  var codeColis =
    valeurPubliquePaiementsAgentsApi_(
      source.codeColis
    );

  var montantAttendu =
    arrondirMontant_(
      nombrePublicPaiementsAgentsApi_(
        source.montantAttendu
      )
    );
