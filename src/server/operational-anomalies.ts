...
export async function readOperationalAnomalies(now = new Date()) {
  ...
  const [payments, cash, storage, orchestrations, closures, forwardings] = await Promise.all([
  ...
  const unavailable = [payments, cash, storage, orchestrations, closures, forwardings].filter((source) => !source.available).length;
  const sourceNames = [["ENCAISSEMENTS", payments], ["CAISSE", cash], ["STOCKAGE", storage], ["ORCHESTRATION", orchestrations], ["CONTINUITE", closures], ["FORWARDING", forwardings]] as const;
