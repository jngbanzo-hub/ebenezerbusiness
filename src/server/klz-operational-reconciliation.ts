...
export type KlzReconciliationKind = "INCREMENTAL" | "DAILY" | "DAILY_RETRY";

const KLZ_RECONCILIATION_ACTOR = Object.freeze({
  userId: "b28fac8a-36c4-4346-85f7-ee168cb8e56b",
  email: "KLZ_RECONCILIATION_READER",
  agency: "KLZ" as const
});
...
export async function runKlzOperationalReconciliation(
  kind: KlzReconciliationKind,
  now = new Date()
) {
  ...
  const { data: claim, error: claimError } = await client.rpc("claim_klz_reconciliation_run", {
  ...
  if (claimError) throw new Error("KLZ_RECONCILIATION_CLAIM_FAILED");
  ...
  try {
    const cursorStart = kind === "INCREMENTAL" && persistedCursorStart === 2
      ? await readAdminPaymentNextRow("KLZ")
    ...
    await client.rpc("fail_klz_reconciliation_run", {
...
export async function readLatestKlzReconciliationReport() {
  const { data, error } = await serviceClient()
    .from("klz_reconciliation_runs")
    .select("run_id,finished_at,anomalies,information,metrics,sources")
    .eq("status", "COMPLETED")
    .order("finished_at", { ascending: false })
    .limit(1)
    .maybeSingle();
  if (error) throw new Error("KLZ_RECONCILIATION_REPORT_UNAVAILABLE");
  const row = record(data);
  if (!text(row.run_id)) throw new Error("KLZ_RECONCILIATION_REPORT_EMPTY");
  return {
...
async function inspect(client: ReturnType<typeof serviceClient>, input: { businessDate: string; cursorStart: number; kind: KlzReconciliationKind; now: Date }) {
  ...
  const [sheet, cash, storage, orchestrations, closures, expenseCash, expenses, telemetry, dailyStorage, storageAccount, previousDaily] = await Promise.all([
    safe("PAYMENTS", async () => {
      const window = await readAdminPaymentWindow("KLZ", sheetStart, SHEET_WINDOW_LIMIT);
      return { rows: window.payments.flatMap((row): ReconciliationPayment[] => row.paymentRequestId && row.agenceEncaissement === "KLZ" && inWindow(row.dateTime, since, input.kind, input.businessDate) ? [{ requestId: row.paymentRequestId, trackingCode: row.codeColis, agency: "KLZ", amountUsd: row.montantPaye, weightKg: row.poidsKg, occurredAt: row.dateTime }] : []), cursorEnd: window.nextRow, paginationIncomplete: window.limitReached };
    }),
    safe("CASH", () => dbRows(client.from("cash_events").select("event_id,source_request_id,agency,amount,occurred_at,metadata").eq("agency", "KLZ").eq("source_type", "PAYMENT_ENGINE").gte("occurred_at", since).order("occurred_at", { ascending: true }).limit(DATABASE_ROW_LIMIT), decodeCash)),
    safe("STORAGE", () => dbRows(client.from("stockage_events").select("event_id,request_id,agency,tracking_code,weight_kg_delta,occurred_at,metadata,source_type,source_request_id").eq("agency", "KLZ").in("event_type", ["SORTIE_APRES_PAIEMENT_TOTAL_DESTINATION", "SORTIE_APRES_REMISE_ACHEMINEMENT"]).gte("occurred_at", since).order("occurred_at", { ascending: true }).limit(DATABASE_ROW_LIMIT), decodeStorage)),
    safe("ORCHESTRATIONS", () => dbRows(client.from("stockage_payment_orchestrations").select("request_id,tracking_code,agency,state,payment_created,cash_event_id,stockage_event_id,last_error,attempt_count,created_at,updated_at,completed_at,parcel_id,forwarding_id").eq("agency", "KLZ").or(`updated_at.gte.${since},state.neq.COMPLETED`).order("updated_at", { ascending: true }).limit(DATABASE_ROW_LIMIT), decodeOrchestration)),
    safe("CLOSURES", () => dbRows(client.from("cash_daily_closures").select("closure_id,agency,business_date,opening_balance,closing_balance,payments_total,expenses_total,status,version,closed_at").eq("agency", "KLZ").gte("business_date", addDays(input.businessDate, -1)).lte("business_date", input.businessDate).order("business_date", { ascending: true }).limit(10), decodeClosure)),
    safe("EXPENSE_CASH", () => dbRows(client.from("cash_events").select("event_id,source_request_id,agency,amount,occurred_at,metadata").eq("agency", "KLZ").eq("source_type", "EXPENSE_ENGINE").gte("occurred_at", expenseWindow.startInclusive).lt("occurred_at", expenseWindow.endExclusive).order("occurred_at", { ascending: true }).limit(DATABASE_ROW_LIMIT), decodeCash)),
    safe("EXPENSES", async () => {
      const response = await readAdminExpenses(KLZ_RECONCILIATION_ACTOR, { agence: "KLZ", dateDebut: expenseWindow.sheetDateStart, dateFin: expenseWindow.sheetDateEnd, page: 1, pageSize: 100 });
      return { rows: response.depenses.flatMap((row): ReconciliationExpense[] => row.devise === "USD" && row.statut === "ACTIVE" && isTimestampInLshiExpenseWindow(row.dateHeure, expenseWindow) ? [{ requestId: row.expenseRequestId, agency: "KLZ", amountUsd: row.montant, occurredAt: row.dateHeure }] : []), paginationIncomplete: response.pagination.totalPages > 1 };
    }),
    safe("TELEMETRY", () => dbRows(client.from("payment_performance_events").select("event_id,agency,total_ms,http_status,timeout,retry,attempt_count,created_at").eq("agency", "KLZ").gte("created_at", new Date(input.now.getTime() - 60 * 60 * 1000).toISOString()).order("created_at", { ascending: true }).limit(1000), decodeTelemetry)),
    input.kind === "INCREMENTAL" ? Promise.resolve({ available: true as const, rows: [] as Array<{ countDelta: number; weightDelta: number }> }) : safe("DAILY_STORAGE_EVENTS", () => dbRows(client.from("stockage_events").select("parcel_count_delta,weight_kg_delta,business_date").eq("agency", "KLZ").eq("business_date", input.businessDate).order("event_id", { ascending: true }).limit(DATABASE_ROW_LIMIT), (row) => ({ countDelta: Number(row.parcel_count_delta), weightDelta: Number(row.weight_kg_delta) }))),
    input.kind === "INCREMENTAL" ? Promise.resolve({ available: true as const, rows: [] as Array<{ parcelCount: number; weightKg: number }> }) : safe("STORAGE_ACCOUNT", () => dbRows(client.from("stockage_accounts").select("current_parcel_count,current_weight_kg").eq("agency", "KLZ").limit(1), (row) => ({ parcelCount: Number(row.current_parcel_count), weightKg: Number(row.current_weight_kg) }))),
    input.kind === "INCREMENTAL" ? Promise.resolve({ available: true as const, rows: [] as Array<{ metrics: Record<string, unknown> }> }) : safe("PREVIOUS_DAILY", () => dbRows(client.from("klz_reconciliation_runs").select("metrics").eq("status", "COMPLETED").in("run_kind", ["DAILY", "DAILY_RETRY"]).lt("business_date", input.businessDate).order("business_date", { ascending: false }).limit(1), (row) => ({ metrics: record(row.metrics) })))
  ]);
...
async function complete(client: ReturnType<typeof serviceClient>, runId: string, cursorEnd: number, anomalies: readonly OperationalAnomaly[], information: readonly OperationalAnomaly[], metrics: Record<string, unknown>, sources: Record<string, unknown>) {
  const { error } = await client.rpc("complete_klz_reconciliation_run", { p_anomalies: anomalies, p_cursor_end: cursorEnd, p_information: information, p_metrics: metrics, p_run_id: runId, p_sources: sources });
  if (error) throw new Error("KLZ_RECONCILIATION_COMPLETE_FAILED");
}
...
async function safe<T>(name: string, reader: () => Promise<{ rows: T[]; cursorEnd?: number; paginationIncomplete?: boolean }>): Promise<Source<T> & { cursorEnd?: number }> {
  ...
  catch (cause) { console.error("[klz-reconciliation-source]", JSON.stringify({ source: name, code: cause instanceof Error ? cause.message : "UNKNOWN" })); return { available: false, rows: [], code: `${name}_UNAVAILABLE` }; }
}
...
function decodeCash(row: Record<string, unknown>): ReconciliationCash { const metadata = record(row.metadata); return { eventId: String(row.event_id), requestId: String(row.source_request_id).toLowerCase(), agency: "KLZ", amountUsd: Number(row.amount), occurredAt: String(row.occurred_at), parcelId: nullable(metadata.parcelId), forwardingId: nullable(metadata.forwardingId) }; }
function decodeStorage(row: Record<string, unknown>): ReconciliationStorage { const metadata = record(row.metadata); return { eventId: String(row.event_id), requestId: String(row.request_id).toLowerCase(), agency: "KLZ", trackingCode: nullable(row.tracking_code), weightKg: row.weight_kg_delta == null ? null : Math.abs(Number(row.weight_kg_delta)), occurredAt: String(row.occurred_at), parcelId: nullable(metadata.parcelId), forwardingId: nullable(metadata.forwardingId) ?? (row.source_type === "INTER_AGENCY_FORWARDING" ? nullable(row.source_request_id) : null) }; }
function decodeOrchestration(row: Record<string, unknown>): ReconciliationOrchestration { return { requestId: String(row.request_id).toLowerCase(), trackingCode: String(row.tracking_code), agency: "KLZ", state: String(row.state), paymentCreated: row.payment_created === true, cashEventId: nullable(row.cash_event_id), storageEventId: nullable(row.stockage_event_id), lastError: nullable(row.last_error), attemptCount: Number(row.attempt_count), createdAt: String(row.created_at), updatedAt: String(row.updated_at), completedAt: nullable(row.completed_at), parcelId: nullable(row.parcel_id), forwardingId: nullable(row.forwarding_id) }; }
function decodeClosure(row: Record<string, unknown>): CashClosure { return { closureId: String(row.closure_id), agency: "KLZ", businessDate: String(row.business_date), openingBalance: Number(row.opening_balance), closingBalance: Number(row.closing_balance), paymentsTotal: Number(row.payments_total), expensesTotal: Number(row.expenses_total), status: String(row.status), version: Number(row.version), occurredAt: String(row.closed_at) }; }
function decodeTelemetry(row: Record<string, unknown>): PaymentPerformanceEvent { return { eventId: String(row.event_id), agency: "KLZ", totalMs: Number(row.total_ms), httpStatus: Number(row.http_status), timeout: row.timeout === true, retry: row.retry === true, attemptCount: Number(row.attempt_count), occurredAt: String(row.created_at) }; }

function technicalAnomaly(now: Date, type: string, category: "PAGINATION" | "PERFORMANCE" | "CONTINUITE", cause: string, severity: "INFO" | "ATTENTION" | "CRITICAL" = "CRITICAL"): OperationalAnomaly { return { id: `klz:${type}`, category, agency: "KLZ", trackingCode: null, paymentRequestId: null, type, occurredAt: now.toISOString(), ageMinutes: 0, interruptedStep: "LECTURE", severity, status: "A_VERIFIER", cause, recommendation: "Auditer uniquement; aucune réparation automatique.", amountUsd: null, weightKg: null, parcelId: null, forwardingId: null, originAgency: null, destinationAgency: null, nature: "INDETERMINE", dossierKey: `KLZ:${type}`, temporalClass: "NOUVELLE_APRES_PROTECTIONS" }; }
...
function inWindow(value: string, since: string, kind: KlzReconciliationKind, businessDate: string) { return kind === "INCREMENTAL" ? value >= since : value.slice(0, 10) === businessDate; }
...
function slot(kind: KlzReconciliationKind, now: Date, businessDate: string) { if (kind !== "INCREMENTAL") return `${kind}:${businessDate}`; const value = new Date(now); value.setUTCMinutes(Math.floor(value.getUTCMinutes() / 10) * 10, 0, 0); return `INCREMENTAL:${value.toISOString()}`; }
