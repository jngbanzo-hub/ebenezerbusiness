...
test("departure quote uses the physical origin identity and never calls payments",()=>{assert.match(departure,/\.eq\("agency", origin\)/);assert.match(departure,/\.is\("forwarding_id", null\)/);assert.match(departure,/parcelId: data\.parcel_id/);assert.doesNotMatch(departure,/paiements-agents|record_cash_payment_credit/);});

test("JL27226 remains isolated from destination listing",()=>{assert.match(departure,/origin: input\.origin/);assert.match(list,/\.eq\("status", "IN_TRANSIT"\)/);});
