...
test("API Admin GET uniquement et filtres complets",()=>{assert.match(route,/authorizeAdminRequest/);assert.match(route,/export async function GET/);assert.doesNotMatch(route,/POST|PUT|PATCH|DELETE/);for(const label of ["INFO","ATTENTION","IMPORTANT","COO","FIH","LSHI","KLZ","QR","STOCKAGE","ENCAISSEMENTS","CAISSE","DÉPENSES","COHÉRENCE COLIS"])assert.match(ui,new RegExp(label));});
