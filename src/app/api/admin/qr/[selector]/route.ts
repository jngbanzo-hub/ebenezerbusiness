...
export async function GET(request:Request,{params}:{params:{selector:string}}){try{const auth=await authorizeAdminRequest(request);if(!auth.authorized)return fail("ACCESS_DENIED",auth.status);const value=await readAdminQr(auth.userId,params.selector);if(!value)return fail("QR_NOT_FOUND",404);return NextResponse.json(value,{headers:{"Cache-Control":"private, no-store"}});}catch(cause){console.error("[admin-qr] GET failed",cause);return fail("QR_SERVICE_UNAVAILABLE",503);}}
