...
export async function GET(request: Request) {
  try {
    ...
    return Response.json(await readDailyReport({ businessDate: businessDatePortoNovo(), agencies: [auth.identity.site], actor: { userId: auth.identity.userId, email: auth.identity.email, agency: auth.identity.site } }), { headers: { "Cache-Control": "private, no-store" } });
