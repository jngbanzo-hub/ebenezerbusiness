...
export async function POST(request: Request) {
  try {
    ...
    const result = await confirmDelivery({ trackingCode: String(body.trackingCode ?? ""), requestId: String(body.requestId ?? ""), physicalConfirmed: body.physicalDeliveryConfirmed === true, actorId: auth.identity.userId, agency: requireStorageAgency(auth.identity.site) });
