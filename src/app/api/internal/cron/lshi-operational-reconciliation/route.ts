...
export async function GET(request: Request) {
  ...
  try {
    const result = await runLshiOperationalReconciliation("INCREMENTAL");
    ...
    return NextResponse.json({ success: false, code: "LSHI_RECONCILIATION_UNAVAILABLE" }, { status: 503, headers: { "Cache-Control": "private, no-store" } });
  }
}
