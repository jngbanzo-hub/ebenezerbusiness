...
export async function GET(request: Request) {
  ...
  try {
    const result = await runKlzOperationalReconciliation("INCREMENTAL");
    ...
    return NextResponse.json({ success: false, code: "KLZ_RECONCILIATION_UNAVAILABLE" }, { status: 503, headers: { "Cache-Control": "private, no-store" } });
  }
}
