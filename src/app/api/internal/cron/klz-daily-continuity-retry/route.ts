...
import { runKlzOperationalReconciliation } from "@/server/klz-operational-reconciliation";
...
export async function GET(request: Request) {
  ...
  try {
    const result = await runKlzOperationalReconciliation("DAILY_RETRY");
    ...
    return NextResponse.json({ success: false, code: "KLZ_DAILY_CONTINUITY_UNAVAILABLE" }, { status: 503, headers: { "Cache-Control": "private, no-store" } });
  }
}
