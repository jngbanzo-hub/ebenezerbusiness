...
type InTransitForwarding = { forwardingId: string; parcelId: string; trackingCode: string; displayCode: string; originAgency: "KLZ"; destinationAgency: "LSHI" | "FIH"; weightKg: number; rateUsdPerKg: number; amountExpectedUsd: number; forwardingReference: string; departedAt: string; status: "IN_TRANSIT" };
type DepartureQuote = { parcelId: string; trackingCode: string; forwardingReference: string; origin: "KLZ"; destination: "LSHI" | "FIH"; weightKg: number; rateUsdPerKg: number; amountExpectedUsd: number };
...
export function AgentStockagesArrivalsPage() {
  ...
  return <Shell back="/agent/stockages" title={`Arrivages — ${data.account.agency}`}>
    ...
    <div className="grid gap-5">
      ...
      {data.forwardingEnabled && (data.account.agency === "LSHI" || data.account.agency === "FIH") ? <InTransitForwardingsPanel onDone={load} /> : null}
    </div>
...
function AgentPhysicalHistory({ mode }: { mode: "outputs" | "statistics" }) {
  ...
  return <Shell back="/agent/stockages" title={`${mode === "outputs" ? "Sorties" : "Statistiques"} — ${data.account.agency}`}><AccountCards accounts={[data.account]} />{mode === "outputs" ? <>{data.forwardingEnabled && data.account.agency === "KLZ" ? <KlzForwardingDeparturePanel onDone={load} /> : null}<ForwardingCommandForm mode="delivery" disabled={!data.actionsEnabled || !data.forwardingEnabled} onDone={load} /><FilteredOutputHistory events={outputs} /></> : <AgentStatisticsView data={data} />}{message && <Notice text={message} />}</Shell>;
}

function KlzForwardingDeparturePanel({ onDone }: { onDone: () => Promise<void> }) {
  const [trackingCode,setTrackingCode]=useState(""); const [destination,setDestination]=useState<"LSHI"|"FIH"|"">(""); const [quote,setQuote]=useState<DepartureQuote|null>(null); const [message,setMessage]=useState(""); const [busy,setBusy]=useState(false); const [requestId,setRequestId]=useState<string|null>(null);
