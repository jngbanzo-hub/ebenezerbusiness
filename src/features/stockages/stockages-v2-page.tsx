...
type InTransitForwarding = { forwardingId: string; parcelId: string; trackingCode: string; displayCode: string; originAgency: "KLZ" | "LSHI"; destinationAgency: "KLZ" | "LSHI" | "FIH"; weightKg: number; rateUsdPerKg: number; amountExpectedUsd: number; forwardingReference: string; departedAt: string; status: "IN_TRANSIT" };
type DepartureQuote = { parcelId: string; trackingCode: string; forwardingReference: string; origin: "KLZ" | "LSHI"; destination: "KLZ" | "LSHI" | "FIH"; weightKg: number; rateUsdPerKg: number; amountExpectedUsd: number };
...
function AgentPhysicalHistory({ mode }: { mode: "outputs" | "statistics" }) {
  ...
  return <Shell back="/agent/stockages" title={`${mode === "outputs" ? "Sorties" : "Statistiques"} — ${data.account.agency}`}><AccountCards accounts={[data.account]} />{mode === "outputs" ? <>{data.forwardingEnabled && (data.account.agency === "KLZ" || data.account.agency === "LSHI") ? <ForwardingDeparturePanel origin={data.account.agency} onDone={load} /> : null}<FilteredOutputHistory events={outputs} /></> : <AgentStatisticsView data={data} />}{message && <Notice text={message} />}</Shell>;
}

function ForwardingDeparturePanel({ origin, onDone }: { origin: "KLZ" | "LSHI"; onDone: () => Promise<void> }) {
  const destinations = origin === "KLZ" ? [{ agency: "LSHI", rate: 13 }, { agency: "FIH", rate: 16 }] as const : [{ agency: "KLZ", rate: 11 }, { agency: "FIH", rate: 13 }] as const;
