...
function AgentPhysicalHistory({ mode }: { mode: "outputs" | "statistics" }) {
  ...
  return <Shell back="/agent/stockages" title={`${mode === "outputs" ? "Sorties" : "Statistiques"} — ${data.account.agency}`}><AccountCards accounts={[data.account]} />{mode === "outputs" ? <>{data.forwardingEnabled && data.account.agency === "KLZ" ? <KlzForwardingDeparturePanel onDone={load} /> : null}<FilteredOutputHistory events={outputs} /></> : <AgentStatisticsView data={data} />}{message && <Notice text={message} />}</Shell>;
}
