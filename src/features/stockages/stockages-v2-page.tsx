...
function QueueSection({ title, section, accountActive, onDelivery }: { title: string; section: "READY" | "PENDING" | "VERIFICATION" | "RECENT"; accountActive: boolean; onDelivery: () => Promise<void> }) {
  ...
  return <Panel title={title}><div className="mb-4 grid gap-3 sm:grid-cols-2"><label className="text-sm">Code colis<input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} className="mt-1 w-full rounded-lg border border-white/15 bg-slate-950 p-2" /></label><label className="text-sm">Site d’encaissement<select value={paymentSite} onChange={(event) => { setPaymentSite(event.target.value); setPage(1); }} className="mt-1 w-full rounded-lg border border-white/15 bg-slate-950 p-2"><option value="ALL">Tous</option><option>COO</option><option>FIH</option><option>LSHI</option><option>KLZ</option></select></label></div>
    ...
    {!response ? <p className="text-slate-400">Chargement…</p> : response.items.length === 0 ? <p className="text-slate-400">Aucun résultat.</p> : <div className="grid gap-3 lg:grid-cols-2">{response.items.map((item) => <article key={item.trackingCode} className="rounded-xl border border-white/10 bg-slate-950/60 p-4"><div className="flex justify-between gap-3"><b>{item.trackingCode}</b><span className="text-xs text-lime-300">{item.deliveryStatus === "DELIVERED" ? "Livré" : item.deliveryStatus === "READY" ? "Prêt" : item.deliveryStatus === "PAYMENT_PENDING" ? "Solde restant" : "Vérification nécessaire"}</span></div><p className="mt-2 text-sm">Bénéficiaire : {item.beneficiary}</p><p className="text-sm">Destination : {item.destination} · Poids : {item.weightKg === null ? "Poids à vérifier" : `${item.weightKg.toFixed(3)} kg`}</p><p className="text-sm">Attendu : {money(item.amountExpected)} · Payé : {money(item.amountPaid)} · Solde : {money(item.remainingBalance)}</p><p className="mt-2 text-xs text-slate-400">{item.paymentLabel}</p>{item.anomalies.length > 0 && <p className="mt-2 text-xs text-amber-300">Anomalie : {item.anomalies.join(", ")}</p>}{section === "READY" && <Button disabled={!item.canConfirmDelivery || pendingCode === item.trackingCode} onClick={() => void deliver(item.trackingCode)} className="mt-3 w-full bg-lime-400 text-slate-950 hover:bg-lime-300">{pendingCode === item.trackingCode ? "Confirmation…" : item.weightState !== "VALID" ? "Poids à vérifier" : accountActive ? "Confirmer la livraison" : "Solde initial requis"}</Button>}{section === "PENDING" && <Link href={`/agent/encaissement?code=${encodeURIComponent(item.trackingCode)}`} className="mt-3 inline-flex w-full justify-center rounded-lg bg-lime-400 px-4 py-2 font-medium text-slate-950">Encaisser le solde</Link>}{section === "VERIFICATION" && <Button disabled className="mt-3 w-full">Vérifier dans Encaissements</Button>}{section === "RECENT" && <p className="mt-3 text-xs text-slate-300">{item.businessDate ?? "—"} · {item.deliveredAt ? new Date(item.deliveredAt).toLocaleString("fr-FR") : "—"} · {item.deliveredBy ?? "—"}</p>}</article>)}</div>}
