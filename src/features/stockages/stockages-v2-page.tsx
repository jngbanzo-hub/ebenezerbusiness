...
type StorageParcel = { trackingCode: string; agency: string; weightKg: number; status: string; arrivedAt: string | null; arrivalAgent: string | null };
type AgentData = { mode: "V2"; account: Account; events: EventRow[]; activity: Activity[]; parcels: StorageParcel[]; actionsEnabled: boolean };
...
async function request<T>(url: string, body?: unknown): Promise<T> { const auth = getSupabaseBrowserClient().auth; const accessToken = body ? await getVerifiedAgentWriteToken(auth) : (await auth.getSession()).data.session?.access_token; if (!accessToken) throw new Error("Session expirée."); const response = await fetch(url, { method: body ? "POST" : "GET", headers: { Authorization: `Bearer ${accessToken}`, ...(body ? { "Content-Type": "application/json" } : {}) }, body: body ? JSON.stringify(body) : undefined, cache: "no-store" }); const payload = await response.json().catch(() => null) as Record<string, unknown> | null; if (!response.ok) throw new Error(typeof payload?.message === "string" ? payload.message : "Service Stockages indisponible."); return payload as T; }
