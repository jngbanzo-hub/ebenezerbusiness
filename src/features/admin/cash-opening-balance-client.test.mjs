...
test("l'interface contient trois saisies séparées sans Request ID visible ni secret serveur",()=>{const ui=readFileSync(new URL("./cash-opening-balance.tsx",import.meta.url),"utf8");const controls=readFileSync(new URL("./cash-admin-controls.tsx",import.meta.url),"utf8");const client=readFileSync(new URL("./cash-opening-balance-client.ts",import.meta.url),"utf8");assert.match(ui,/\["FIH", "LSHI", "KLZ"\]/);assert.match(ui,/confirmationFinal: true/);assert.match(`${ui}\n${controls}`,/createCashRequestId/);assert.doesNotMatch(`${ui}\n${controls}`,/Request ID|placeholder=["']Request ID/);assert.doesNotMatch(`${ui}\n${controls}\n${client}`,/SUPABASE_SERVICE_ROLE_KEY/);});
