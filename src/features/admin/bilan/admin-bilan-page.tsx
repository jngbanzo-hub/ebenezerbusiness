...
type ModernDebtRow = { code:string; exactPrefix:string; sourceSheet:string; date:string|null; year:number|null; cohort:string|null; weightKg:number|null; sender:string|null; beneficiary:string|null; manifestF:unknown; manifestL?:unknown; expectedUsd:number|null; paidUsd:number; remainingUsd:number|null; financialState:string; state:string; reason:string; transactions:Array<{amountUsd:number;status:string;date:string;collectingAgency:string;destination:string;paymentRequestId:string|null}>; lastPaymentDate:string|null; lastCollectingAgency:string|null; physicalMatches?:Array<{agency:string;trackingCode:string;parcelId:string|null;forwardingId:string|null;originAgency:string|null;destinationAgency:string|null;weightKg:number|null;deliveryStatus:string|null;paymentRequestId:string|null;orchestrationState:string|null;createdAt:string|null;currentlyPresent?:boolean;everPresent?:boolean}> };
...
function ModernPaymentsCardsLegacy({ audit }: { audit: ModernAudit }) {
  ...
  const [previewRows, setPreviewRows] = useState<ModernDebtRow[]>([]);
  useEffect(() => { setCohortFilter(audit.activeCohortId ?? "ALL"); setAgencyFilter("ALL"); setStatusFilter("DEBT"); setSearch(""); setOpen(null); }, [audit.activeCohortId]);
  const cohorts = audit.activeCohortId ? [audit.activeCohortId] : Object.keys(audit.byCohort);
  const rows = audit.rows.filter(row => (agencyFilter === "ALL" || row.sourceSheet === agencyFilter) && (cohortFilter === "ALL" || row.cohort === cohortFilter) && (statusFilter === "DEBT" ? row.state === "NON PAYÉ" || row.state === "PARTIEL" : statusFilter === "FUTURE" ? row.state === "FUTURE DETTE" : statusFilter === "ISOLATED" ? row.state === "CAS_ISOLE_PREUVE_PHYSIQUE_INSUFFISANTE" : statusFilter === "VERIFY" ? row.state === "À VÉRIFIER" : true) && row.code.toLowerCase().includes(search.trim().toLowerCase()));
  const reminderCandidates = rows.filter(isReminderEligible);
  const reminderKey = (row: ModernDebtRow) => `${row.sourceSheet}:${row.cohort ?? "UNRESOLVED"}:${row.code}`;
  const selectEligible = () => setSelectedReminderKeys(reminderCandidates.map(reminderKey));
  const previewNotifications = () => setPreviewRows(reminderCandidates.filter(row => selectedReminderKeys.includes(reminderKey(row))));
  return <><div className="mt-4 grid gap-3 rounded-lg border border-white/10 p-4 md:grid-cols-4"><label className="text-sm">Agence<select className={field} value={agencyFilter} onChange={event=>setAgencyFilter(event.target.value)}><option value="ALL">Toutes</option>{agencies.map(agency=><option key={agency}>{agency}</option>)}</select></label><label className="text-sm">Cohorte<select className={field} value={cohortFilter} onChange={event=>setCohortFilter(event.target.value)}><option value="ALL">Toutes</option>{cohorts.map(cohort=><option key={cohort}>{cohort}</option>)}</select></label><label className="text-sm">État<select className={field} value={statusFilter} onChange={event=>setStatusFilter(event.target.value)}><option value="DEBT">Dettes actuelles</option><option value="FUTURE">Futures dettes</option><option value="VERIFY">À vérifier</option><option value="ALL">Tous</option></select></label><label className="text-sm">Recherche par code<input className={field} value={search} onChange={event=>setSearch(event.target.value)} placeholder="AT14526"/></label></div>{statusFilter !== "VERIFY" && <div className="mt-4 rounded-lg border border-amber-400/30 bg-amber-950/20 p-4 text-sm"><p className="font-semibold text-amber-200">RAPPELS WHATSAPP — DRY-RUN</p><p className="mt-1 text-muted-foreground">Aucun envoi, appel 360dialog ou réservation n’est possible.</p><div className="mt-3 flex flex-wrap gap-2"><Button type="button" variant="outline" onClick={selectEligible} disabled={!reminderCandidates.length}>Tout sélectionner les éligibles ({reminderCandidates.length})</Button><Button type="button" variant="ghost" onClick={previewNotifications} disabled={!selectedReminderKeys.length}>Prévisualiser les notifications ({selectedReminderKeys.length})</Button></div><p className="mt-2 text-xs text-muted-foreground">Candidats certifiés (bénéficiaire ou expéditeur) avec téléphone exploitable : {reminderCandidates.length}</p></div>}{previewRows.length ? <div className="mt-4 rounded-lg border border-white/10 p-4"><h4 className="font-semibold">Prévisualisation des notifications (lecture seule)</h4><div className="mt-3 space-y-3">{previewRows.map(row=>{const recipients=buildReminderRecipients(row); return <div key={reminderKey(row)} className="rounded border border-white/10 p-3 text-sm"><p><strong>CODE :</strong> {row.code} · <strong>AGENCE :</strong> {row.lastCollectingAgency ?? row.sourceSheet} · <strong>COHORTE :</strong> {row.cohort}</p><p><strong>DESTINATAIRES UNIQUES :</strong> {recipients.filter(item=>item.eligible).length}</p>{recipients.map(recipient=><p key={`${recipient.recipientType}:${recipient.normalizedPhone??recipient.name}`}><strong>{recipient.roles.length > 1 ? "EXPÉDITEUR + BÉNÉFICIAIRE" : recipient.recipientType === "SENDER" ? "EXPÉDITEUR" : "BÉNÉFICIAIRE"} :</strong> {recipient.name ?? "NON DISPONIBLE"} · <strong>TÉLÉPHONE :</strong> {recipient.normalizedPhone ?? "NON DISPONIBLE"} · <strong>ÉLIGIBLE :</strong> {recipient.eligible ? "OUI" : "NON"}</p>)}<p><strong>ÉTAT :</strong> {row.state} · <strong>PAYÉ :</strong> {usd(row.paidUsd)} · <strong>RESTE :</strong> {usd(row.remainingUsd ?? 0)}</p><p className="mt-2 text-muted-foreground">Prévisualisation uniquement — aucun message n’est envoyé.</p></div>})}</div></div> : null}<div className="mt-4 grid gap-4 xl:grid-cols-3">{agencies.map(agency=>{const item=audit.byAgency[agency]; const agencyRows=rows.filter(row=>row.sourceSheet===agency); const debts=statusFilter==="FUTURE"?item.futureDebtRows:item.debts; const openRows=agencyRows.length?agencyRows:[]; const recipientStats=debts.reduce((stats,row)=>{const recipients=buildReminderRecipients(row); const eligible=recipients.filter(item=>item.eligible); stats.raw+=recipients.reduce((sum,item)=>sum+item.roles.length,0); stats.unique+=eligible.length; stats.beneficiaries+=eligible.some(item=>item.roles.includes("BENEFICIARY"))?1:0; stats.senders+=eligible.some(item=>item.roles.includes("SENDER"))?1:0; stats.both+=eligible.some(item=>item.roles.length>1)?1:0; return stats;},{raw:0,unique:0,beneficiaries:0,senders:0,both:0}); return <GlassPanel key={agency} className="p-5"><h4 className="text-lg font-semibold text-accent">{agency}</h4><dl className="mt-4 space-y-2 text-sm"><div className="flex justify-between"><dt>Codes totaux</dt><dd>{item.total}</dd></div><div className="flex justify-between"><dt>Codes encaissés / SOLDÉS</dt><dd>{item.settled}</dd></div><div className="flex justify-between"><dt>Codes PARTIELS ACTUELS</dt><dd>{item.partial}</dd></div><div className="flex justify-between"><dt>Dettes actuelles</dt><dd>{item.currentDebts}</dd></div><div className="flex justify-between"><dt>Futures dettes</dt><dd>{item.futureDebts}</dd></div><div className="flex justify-between"><dt>À vérifier</dt><dd>{item.toVerify}</dd></div></dl><div className="mt-4 border-t border-white/10 pt-3 text-sm"><p>Total encaissé certifié : <strong>{usd(item.certifiedPaidUsd)}</strong></p><p className="mt-1">TOTAL RESTANT : <strong>{item.certifiedRemainingUsd===null?"NON CERTIFIABLE":usd(item.certifiedRemainingUsd)}</strong></p><p className="mt-2">Bénéficiaires éligibles : <strong>{recipientStats.beneficiaries}</strong></p><p>Expéditeurs éligibles : <strong>{recipientStats.senders}</strong></p><p>Les deux : <strong>{recipientStats.both}</strong></p><p>Destinataires bruts / uniques : <strong>{recipientStats.raw} / {recipientStats.unique}</strong></p></div><Button type="button" variant="outline" className="mt-4 w-full" disabled={!debts.length} onClick={()=>{setAgencyFilter(agency);setStatusFilter(statusFilter==="FUTURE"?"FUTURE":"DEBT");setOpen(open===agency?null:agency);}}>{open===agency?"Masquer":statusFilter==="FUTURE"?"Voir les futures dettes":"Voir les dettes actuelles"} ({debts.length})</Button><Button type="button" variant="ghost" className="mt-2 w-full" onClick={()=>{setAgencyFilter(agency);setStatusFilter("VERIFY");setOpen(agency);}}>Voir les codes à vérifier ({item.toVerify})</Button>{open===agency?<div className="mt-4 max-h-[32rem] overflow-auto space-y-2">{openRows.length?openRows.map((row,index)=><div key={`${agency}:${row.code}:${index}`} className="rounded border border-white/10 p-3 text-xs"><p className="font-semibold">{row.code} · {row.sourceSheet} · {row.cohort??"NON CERTIFIABLE"}</p><p className="mt-1">Date : {row.date??"NON DISPONIBLE"} · Poids : {row.weightKg===null?"NON DISPONIBLE":`${row.weightKg} kg`} · Destination : {row.sourceSheet}</p><p className="mt-1">{buildReminderRecipients(row).map(recipient=><span key={`${recipient.recipientType}:${recipient.normalizedPhone??recipient.name}`} className="mr-3">{recipient.roles.length > 1 ? "Expéditeur + Bénéficiaire" : recipient.recipientType === "SENDER" ? "Expéditeur" : "Bénéficiaire"} : {recipient.name??"NON DISPONIBLE"} · {recipient.normalizedPhone??"NON DISPONIBLE"}</span>)}</p><p className="mt-1">Attendu : {row.expectedUsd===null?"NON CERTIFIABLE":usd(row.expectedUsd)} · Payé : {usd(row.paidUsd)} · Reste : {row.remainingUsd===null?"NON CERTIFIABLE":usd(row.remainingUsd)}</p><p className="mt-1">Stockage V2 : {row.physicalMatches?.length ? row.physicalMatches.map(match=>`${match.agency} · ${match.deliveryStatus??"STATUT INCONNU"} · ${match.currentlyPresent ? "PRÉSENT" : match.everPresent ? "HISTORIQUE" : "ABSENT"}`).join(" | ") : "ABSENT / AUCUNE PREUVE"}</p><p className="mt-1">Statut : {row.state} · Destinataires éligibles : {buildReminderRecipients(row).filter(item=>item.eligible).length} · Éligible WhatsApp : {isReminderEligible(row)?"OUI":"NON"}</p><p className="mt-1">Type : {buildReminderRecipients(row).filter(item=>item.eligible).length ? "BENEFICIARY_OR_SENDER" : "NON_ELIGIBLE"}</p></div>):<p className="text-sm text-muted-foreground">Aucun résultat pour ces filtres.</p>}</div>:null}</GlassPanel>})}</div></>;
}

type ReminderRecipient = { recipientType: "SENDER" | "BENEFICIARY"; roles: Array<"SENDER" | "BENEFICIARY">; name: string | null; rawPhone: string | null; normalizedPhone: string | null; phoneAvailable: boolean; eligible: boolean; exclusionReason: string | null };
...
function buildReminderRecipients(row: ModernDebtRow): ReminderRecipient[] {
  const certifiedDebt = (row.state === "NON PAYÉ" || row.state === "PARTIEL") && row.remainingUsd !== null && row.remainingUsd > 0;
  const source = ([
    ["SENDER", row.sender],
    ["BENEFICIARY", row.beneficiary]
  ] as const).map(([recipientType, raw]) => {
    const parsed = splitContact(raw);
    const normalizedPhone = normaliseReminderPhone(parsed.phone);
    return { recipientType, roles: [recipientType], name: parsed.name, rawPhone: parsed.phone, normalizedPhone, phoneAvailable: normalizedPhone !== null, eligible: certifiedDebt && normalizedPhone !== null, exclusionReason: !certifiedDebt ? "DEBT_NOT_CERTIFIED" : normalizedPhone === null ? "PHONE_ABSENT_OR_INVALID" : null } satisfies ReminderRecipient;
  });
  const byPhone = new Map<string, ReminderRecipient>();
  source.forEach((recipient) => {
    if (recipient.normalizedPhone && byPhone.has(recipient.normalizedPhone)) {
      const existing = byPhone.get(recipient.normalizedPhone);
      if (existing && !existing.roles.includes(recipient.recipientType)) existing.roles.push(recipient.recipientType);
      return;
    }
    byPhone.set(recipient.normalizedPhone ?? `${recipient.recipientType}:${recipient.name ?? "UNKNOWN"}`, recipient);
  });
  return Array.from(byPhone.values());
}

function isReminderEligible(row: ModernDebtRow) {
  return buildReminderRecipients(row).some((recipient) => recipient.eligible);
}
