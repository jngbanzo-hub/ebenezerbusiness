...
test("tableau de direction contient exactement huit cartes de synthèse",()=>{for(const text of ["Activité du mois d’origine","Expéditions du mois d’origine","Encaissements","Charges directes","Dépenses opérationnelles","Trésorerie","Résultat financier","Qualité des données"])assert.ok(ui.includes(`title:\"${text}\"`),text);assert.match(ui,/sm:grid-cols-2 xl:grid-cols-4/);assert.match(ui,/cards\.map\(/);});
...
test("résultat financier n’invente ni chiffre d’affaires ni bénéfice",()=>{assert.match(ui,/Chiffre d’affaires réel/);assert.match(ui,/Bénéfice réel : NON CALCULABLE/);assert.match(ui,/BÉNÉFICE RÉEL/);assert.match(ui,/EN ATTENTE DE CERTIFICATION/);});
