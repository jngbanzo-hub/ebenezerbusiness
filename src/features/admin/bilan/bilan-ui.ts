...
export type BilanPayload = {
  ...
  airFreight: { status: "CERTIFIE" | "PROVISOIRE"; byAgency: Record<"FIH" | "LSHI" | "KLZ", number>; byAgencyCompany: Record<"FIH" | "LSHI", Record<"ETHIOPIAN" | "ASKY" | "DHL", number>>; totalUsd: number; physicalMonthUsd: number; laterMonthUsd: number; allocations: Array<{ sourceReference: string; date: string; company: string; destination: "FIH" | "LSHI"; cohortWeightKg: number; amountUsd: number; officialAmountUsd: number; physicalMonth: string }>; anomalies: string[] };
