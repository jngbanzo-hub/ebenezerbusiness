...
export function AdminExpensesModule({ accessToken }: { accessToken: string }) {
  ...
  const totals = useMemo(() => ({
    USD: response?.totaux.parDevise.USD ?? 0,
    FCFA: response?.totaux.parDevise.FCFA ?? 0,
    CDF: response?.totaux.parDevise.CDF ?? 0
  }), [response]);
  ...
  return <section className="mt-8 space-y-6">
    <GlassPanel className="p-5 sm:p-6">
      ...
      <form className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4" onSubmit={applyFilters}>
        ...
        <Field label="Agence"><select className={fieldClassName} value={draft.agency} onChange={(event) => setDraft({ ...draft, agency: event.target.value as FilterForm["agency"] })}><option className="bg-ebe-navy" value="">Toutes</option>{agencies.map((agency) => <option className="bg-ebe-navy" key={agency}>{agency}</option>)}</select></Field>
        <Field label="Catégorie"><input className={fieldClassName} value={draft.category ?? ""} onChange={(event) => setDraft({ ...draft, category: event.target.value || undefined })} placeholder="Toutes les catégories" /></Field>
        ...
        <Field label="Agent"><input className={fieldClassName} value={draft.agent ?? ""} onChange={(event) => setDraft({ ...draft, agent: event.target.value || undefined })} placeholder="Nom de l’agent" /></Field>
    ...
    {response ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><TotalCard label="Nombre de dépenses" value={String(response.totaux.nombreDepenses)} /><TotalCard label="Total USD" value={formatAmount(totals.USD, "USD")} /><TotalCard label="Total FCFA" value={formatAmount(totals.FCFA, "FCFA")} /><TotalCard label="Total CDF" value={formatAmount(totals.CDF, "CDF")} /></div> : null}
