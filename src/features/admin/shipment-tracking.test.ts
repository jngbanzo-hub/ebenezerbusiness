...
test("autorise uniquement le nouveau menu", () => { assert.equal(SHIPMENT_STATUSES.includes("En Transit" as never), false); assert.equal(isShipmentStatus("En Transit à Addis"), true); assert.equal(isShipmentStatus("Arrivé à KLZ"), true); assert.equal(isShipmentStatus("En Transit"), false); });
...
test("crée L uniquement lors d'une vraie transition vers Arrivé avec L vide", () => {
  ...
  assert.equal(shouldCreateArrivalDate("Arrivé", "10/08/2026", "Arrivé à KLZ"), false);
