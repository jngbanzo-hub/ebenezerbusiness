...
export function AdminGlobalParcelSearch() {
  ...
  return <main className="min-h-screen bg-ebe-night py-8 text-white"><Container className="max-w-6xl">
    ...
    {result?<section className="mt-6 space-y-5"><GlassPanel className="p-6"><p className="text-sm text-muted-foreground">Code recherché</p><h2 className="mt-1 text-2xl font-bold">{result.code}</h2><p className="mt-2 text-accent">{result.found?"INFORMATIONS TROUVÉES":"AUCUN COLIS TROUVÉ"}</p>{result.found?<Button asChild variant="growth" className="mt-4"><Link href={`/admin/colis/${encodeURIComponent(result.code)}/historique`}>Voir l’historique complet</Link></Button>:null}</GlassPanel>
      <div className="grid gap-5 lg:grid-cols-2"><SourceCard title="MANIFESTE" source={result.manifest} render={(item)=><><p>Destination : {item.agency}</p><p>Date : {item.date||"—"}</p><p>Poids : {item.weightKg??"—"} kg</p><p>Statut : {item.status}</p><p>Ligne : {item.rowNumber}</p></>}/><SourceCard title="EXPÉDITION" source={result.shipments} emptyLabel="Aucune information d’expédition trouvée" render={(item)=><><p className="font-semibold text-accent">{item.isLatestForAgency?"Groupage principal (le plus récent)":"Groupage historique"}</p><p>Code : {item.code}</p><p>Agence : {item.agency}</p><p>Destination : {item.destination}</p><p>Groupage : {item.groupage||"Non renseigné"}</p><p>Date d’expédition : {formatDate(item.date)}</p><p>Compagnie : {item.company||"—"}</p><p>Statut groupage : {item.status||"—"}</p></>}/><SourceCard title="STOCKAGE V2" source={result.storage} render={(item)=><><p>Agence : {item.agency}</p><p>Poids : {item.weightKg} kg</p><p>État : {item.status}</p><p>Dernière mise à jour : {item.updatedAt}</p><p>Dernier événement : {item.lastEvent?.type??"—"}</p></>}/><SourceCard title="ENCAISSEMENTS" source={result.payments} render={(item)=><><p>Montant attendu : {item.montantAttendu??"—"}</p><p>Montant payé : {item.montantPaye}</p><p>Statut : {item.statutPaiement}</p><p>Agence : {item.agenceEncaissement}</p><p>Date : {item.dateTime}</p><p>Mode : {item.modePaiement||"—"}</p><p>Référence : {item.reference||"—"}</p></>}/><SourceCard title="QR" source={result.qr} render={(item)=><><p>QR visible : {String(item.displayNumber).padStart(3,"0")}</p><p>qrId : {item.qrId}</p><p>Statut : {item.status}</p><p>Version : {item.version}</p><p>Association : {item.agency??"—"} + {item.trackingCode??"—"}</p><p>Date d’association : {item.assignedAt??"—"}</p><p>Audit : {item.audit.map((event)=>event.action).join(", ")||"—"}</p></>}/></div>
    </section>:null}
  </Container></main>;
}

function SourceCard<T>({title,source,render,emptyLabel}:{title:string;source:{state:keyof typeof sourceLabel;matches:T[]};render:(item:T)=>React.ReactNode;emptyLabel?:string}) { return <GlassPanel className="p-6"><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="text-xl font-semibold">{title}</h3><span className="rounded-full border border-white/15 px-3 py-1 text-xs font-bold">{sourceLabel[source.state]}</span></div>{source.matches.length>1?<p className="mt-3 font-bold text-amber-200">PLUSIEURS CORRESPONDANCES TROUVÉES</p>:null}{source.state==="ABSENT"&&emptyLabel?<p className="mt-4 text-sm text-muted-foreground">{emptyLabel}</p>:null}<div className="mt-4 space-y-3 text-sm">{source.matches.map((item,index)=><div key={index} className="rounded-lg border border-white/10 bg-white/5 p-4">{render(item)}</div>)}</div></GlassPanel>; }
function formatDate(value:string) { const date=new Date(`${value}T00:00:00Z`); return Number.isNaN(date.getTime())?value:new Intl.DateTimeFormat("fr-FR",{timeZone:"UTC"}).format(date); }
