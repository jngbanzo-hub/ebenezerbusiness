...
type FormState = { amount: string; businessDate: string; observation: string; confirmed: boolean };
const empty = (): FormState => ({ amount: "", businessDate: "", observation: "", confirmed: false });
...
function OpeningBalanceForm({ agency, accessToken }: { agency: Agency; accessToken: string }) {
  ...
  const requestId = useRef(createCashRequestId());
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (lock.current) return;
    const amount = Number(form.amount);
    if (!Number.isFinite(amount) || amount <= 0 || !form.businessDate || !form.confirmed) { setFeedback({ ok: false, text: "Montant, date métier et confirmation sont obligatoires." }); return; }
    ...
    try {
      const result = await submitOpeningBalance(accessToken, { agency, amount, businessDate: form.businessDate, observation: form.observation.trim() || undefined, requestId: requestId.current, confirmationFinal: true });
      ...
      requestId.current = createCashRequestId();
