...
type Report = {
  ...
  fZeroAudit?: Record<string, { total: number; withP1: number; settled: number; partial: number; ambiguous: number; absent: number; duplicates: number; forwarding: number; rows: Array<{ code: string; destination: string; cohort: string | null; weightKg: number | null; manifestF: unknown; manifestG: unknown; manifestL: unknown; totalPaidUsd: number; classification: string; duplicateIds: string[]; transactions: Array<{ amountUsd: number; status: string; date: string; agency: string; destination: string; paymentRequestId: string | null }> }> }>;
  ...
  modernManifestAudit?: ModernManifestAudit;
};
...
type ModernManifestRow = {
  ...
  lastPaymentDate: string | null; lastCollectingAgency: string | null;
  physicalMatches: Array<{ parcelId: string | null; forwardingId: string | null; originAgency: string | null; destinationAgency: string | null; deliveryStatus: string | null; orchestrationState: string | null }>;
};
type ModernManifestAudit = {
  startDate: string; endDate: string; rows: ModernManifestRow[];
  byAgency: Record<string, { total: number; settled: number; partial: number; unpaidCertified: number; toVerify: number; certifiedPaidUsd: number; certifiedRemainingUsd: number | null; debts: ModernManifestRow[] }>;
  byCohort: Record<string, { total: number; settled: number; partial: number; unpaidCertified: number; toVerify: number }>;
  conservation: Record<string, string>;
};
...
export function EncaissementsCertificationReadonlyPage() {
  ...
  const modernCohortByAgency = modernManifestAudit ? Object.entries(modernManifestAudit.rows.reduce<Record<string, Record<string, { total: number; settled: number; partial: number; unpaidCertified: number; toVerify: number }>>>((acc, row) => {
    const cohort = row.cohort ?? `${row.exactPrefix || "UNKNOWN"}-${row.year ?? "UNKNOWN"}`;
    const current = acc[cohort]?.[row.sourceSheet] ?? { total: 0, settled: 0, partial: 0, unpaidCertified: 0, toVerify: 0 };
    current.total += 1;
    if (row.state === "SOLDÉ") current.settled += 1;
    else if (row.state === "PARTIEL") current.partial += 1;
    else if (row.state === "NON PAYÉ") current.unpaidCertified += 1;
    else current.toVerify += 1;
    return { ...acc, [cohort]: { ...(acc[cohort] ?? {}), [row.sourceSheet]: current } };
  }, {})) : [];
  ...
  return <main className="min-h-screen bg-ebe-night py-8 text-white"><Container>
    ...
    {report ? <div className="mt-6 grid gap-5">
      ...
      <GlassPanel className="p-5"><h2 className="text-xl">Identité physique AT02326</h2><p className="mt-2 text-sm text-muted-foreground">Résolution read-only par parcel_id, forwarding_id, agence et orchestration ; le tracking seul n’est pas une identité suffisante.</p><div className="mt-3 grid gap-2">{report.physicalIdentities?.state !== "FOUND" ? <p className="text-amber-100">Source physique temporairement indisponible / non certifiable.</p> : report.physicalIdentities.matches.map((match, index) => <div key={`${match.agency}:${match.parcelId ?? match.forwardingId ?? index}`} className="rounded border border-white/10 p-3"><strong>{match.trackingCode} · {match.agency}</strong><p className="text-xs text-muted-foreground">Nature : {match.forwardingId ? "FORWARDING" : match.parcelId ? "NATIF" : "NON RÉSOLUE"} · parcel_id : {match.parcelId ?? "NULL"} · forwarding_id : {match.forwardingId ?? "NULL"}</p><p className="text-xs text-muted-foreground">Route : {match.originAgency ?? "—"} → {match.destinationAgency ?? "—"} · poids : {match.weightKg ?? "N/C"} kg · statut : {match.deliveryStatus ?? "N/C"}</p><p className="text-xs text-muted-foreground">paymentRequestId : {match.paymentRequestId ?? "N/C"} · orchestration : {match.orchestrationState ?? "N/C"}</p></div>)}</div></GlassPanel>
      {modernManifestAudit ? <GlassPanel className="p-5"><h2 className="text-xl">Audit inversé Manifeste COO → P1 — lecture seule</h2><p className="mt-2 text-sm text-muted-foreground">Périmètre dynamique : {modernManifestAudit.startDate} → {modernManifestAudit.endDate}. Aucune dette n’est injectée dans les compteurs officiels du Bilan.</p>
        <div className="mt-3 grid gap-3">{Object.entries(modernManifestAudit.byAgency).map(([agency, item]) => <div key={agency} className="rounded border border-white/10 p-3"><strong>{agency}</strong><p className="text-sm">Identités manifeste modernes : {item.total} · SOLDÉS : {item.settled} · PARTIELS : {item.partial} · NON PAYÉS CERTIFIÉS : {item.unpaidCertified} · À VÉRIFIER : {item.toVerify}</p><p className="text-xs text-muted-foreground">Total encaissé certifié : {item.certifiedPaidUsd} USD · Total restant certifié : {item.certifiedRemainingUsd === null ? "NON CERTIFIABLE" : `${item.certifiedRemainingUsd} USD`} · Conservation : {modernManifestAudit.conservation[agency] ?? "N/C"}</p><details className="mt-2"><summary className="cursor-pointer text-sm text-accent">Codes restant à encaisser ({item.debts.length})</summary><div className="mt-2 max-h-[28rem] overflow-auto space-y-2">{item.debts.map((row, index) => <div key={`${agency}:${row.code}:${index}`} className="rounded border border-white/10 p-2 text-xs"><p><strong>{row.code}</strong> · cohorte {row.cohort ?? "N/C"} · date {row.date ?? "N/C"} · destination commerciale {row.sourceSheet}</p><p>Poids : {row.weightKg ?? "N/C"} kg · bénéficiaire : {row.beneficiary ?? "N/C"} · téléphone disponible : NON DISPONIBLE</p><p>F Manifeste : {formatRaw(row.manifestF)} · montant attendu certifié : {row.expectedUsd ?? "N/C"} USD · payé cumulé : {row.paidUsd} USD · reste certifié : {row.remainingUsd ?? "N/C"} USD</p><p>État : {row.state} · nature : à résoudre via identité physique · raison : {row.reason}</p>{row.transactions.length ? <p className="text-muted-foreground">P1 : {row.transactions.map(transaction => `${transaction.collectingAgency} ${transaction.amountUsd} USD ${transaction.status || "N/C"} ${transaction.date}`).join(" | ")}</p> : <p className="text-muted-foreground">P1 trouvé : NON</p>}</div>)}</div></details></div>)}</div>
        <div className="mt-4 rounded border border-white/10 p-3"><h3 className="font-medium">Ventilation dynamique par cohorte et agence</h3><div className="mt-2 grid gap-2">{modernCohortByAgency.map(([cohort, agencies]) => <p key={cohort} className="text-xs text-muted-foreground"><strong>{cohort}</strong> · {Object.entries(agencies).map(([agency, item]) => `${agency} total ${item.total} / SOLDÉS ${item.settled} / PARTIELS ${item.partial} / NON PAYÉS ${item.unpaidCertified} / À VÉRIFIER ${item.toVerify}`).join(" · ")}</p>)}</div></div>
        <div className="mt-4 rounded border border-white/10 p-3"><h3 className="font-medium">À VÉRIFIER — exclus des dettes</h3><div className="mt-2 max-h-[24rem] overflow-auto space-y-2">{modernManifestAudit.rows.filter(row => row.state === "À VÉRIFIER").map((row, index) => <div key={`verify:${row.code}:${index}`} className="rounded border border-amber-100/20 p-2 text-xs"><strong>{row.code}</strong> · destination {row.sourceSheet} · cohorte {row.cohort ?? "N/C"} · raison : {row.reason} · ambiguïté : {row.state} · forwarding : NON DÉTERMINÉ · P1 trouvé : {row.transactions.length ? "OUI" : "NON"}</div>)}</div></div>
        <div className="mt-4 rounded border border-white/10 p-3"><h3 className="font-medium">AT09826 — identité et rapprochement</h3>{modernManifestAudit.rows.filter(row => row.code === "AT09826").map((row, index) => <div key={`at09826:${index}`} className="mt-2 rounded border border-white/10 p-2 text-xs"><p><strong>AT09826</strong> · origine/agence financière actuelle : {row.sourceSheet} · cohorte {row.cohort ?? "N/C"} · état financier : {row.state} · dette : {row.state === "PARTIEL" || row.state === "NON PAYÉ" ? "OUI" : row.state === "À VÉRIFIER" ? "À VÉRIFIER" : "NON"}</p><p>F Manifeste : {formatRaw(row.manifestF)} · P1 trouvé : {row.transactions.length ? "OUI" : "NON"} · raison : {row.reason}</p>{row.physicalMatches.map((match, physicalIndex) => <p key={physicalIndex}>Identité {match.forwardingId ? "FORWARDING" : match.parcelId ? "NATIVE" : "NON RÉSOLUE"} · route {match.originAgency ?? "—"} → {match.destinationAgency ?? "—"} · forwarding_id {match.forwardingId ?? "NULL"} · parcel_id {match.parcelId ?? "NULL"} · statut acheminement {match.orchestrationState ?? "N/C"} · arrival_confirmed : {match.deliveryStatus === "DELIVERED" ? "OUI" : "NON"} · agence financière : {row.sourceSheet}</p>)}</div>)}</div>
      </GlassPanel> : null}
      ...
      <GlassPanel className="p-5"><h2 className="text-xl">Rapprochement direct F_ZERO → P1</h2><p className="mt-2 text-sm text-muted-foreground">Index exact du code dans COO, FIH, LSHI et KLZ. Les transactions sont dédupliquées par paymentRequestId lorsque disponible.</p><div className="mt-3 grid gap-3">{Object.entries(report.fZeroAudit ?? {}).map(([agency, item]) => <div key={agency} className="rounded border border-white/10 p-3"><strong>{agency}</strong><p className="text-xs text-muted-foreground">Total : {item.total} · avec P1 : {item.withP1} · SOLDÉ certifié : {item.settled} · PARTIEL certifié : {item.partial} · ambigu : {item.ambiguous} · sans P1 : {item.absent} · duplications : {item.duplicates} · forwarding à désambiguïser : {item.forwarding}</p><details className="mt-2"><summary className="cursor-pointer text-sm text-accent">Voir les rapprochements ({item.rows.length})</summary><div className="mt-2 max-h-[32rem] overflow-auto space-y-2">{item.rows.map(row => <div key={`${agency}:${row.code}`} className="rounded border border-white/10 p-2 text-xs"><p><strong>{row.code}</strong> · destination {row.destination} · cohorte {row.cohort ?? "N/C"} · poids {row.weightKg ?? "N/C"} kg · classification {row.classification}</p><p>F : {formatRaw(row.manifestF)} · G : {formatRaw(row.manifestG)} · L : {formatRaw(row.manifestL)} · total P1 : {row.totalPaidUsd} USD</p><p>{row.transactions.length ? row.transactions.map(transaction => `${transaction.agency} ${transaction.amountUsd} USD ${transaction.status || "statut N/C"} ${transaction.date} ${transaction.paymentRequestId ?? "requestId N/C"}`).join(" | ") : "Aucune transaction P1 trouvée"}</p></div>)}</div></details></div>)}</div></GlassPanel>
