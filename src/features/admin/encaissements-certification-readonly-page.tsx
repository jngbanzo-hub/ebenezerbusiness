...
type Report = {
  ...
  p1ModernAudit?: Record<string, { rowCount: number; grossExpectedUsd: number; grossPaidUsd: number; grossRemainingUsd: number; settledCount: number; partialCount: number; otherStatusCount: number; certifiedRowCount: number; certifiedPaidUsd: number; excludedCount: number; excludedAmountUsd: number; exclusionReasons: Record<string, { count: number; amount: number }> }>;
};
...
export function EncaissementsCertificationReadonlyPage() {
  ...
  return <main className="min-h-screen bg-ebe-night py-8 text-white"><Container>
    ...
    {report ? <div className="mt-6 grid gap-5">
      ...
      <GlassPanel className="p-5"><h2 className="text-xl">Audit brut P1 — AT 2026 et après</h2><div className="mt-3 grid gap-2">{Object.entries(report.p1ModernAudit ?? {}).map(([agency, item]) => <div key={agency} className="rounded border border-white/10 p-3"><strong>{agency}</strong><p className="text-sm">Lignes : {item.rowCount} · attendu brut : {item.grossExpectedUsd} USD · payé brut : {item.grossPaidUsd} USD · reste brut : {item.grossRemainingUsd} USD</p><p className="text-xs text-muted-foreground">SOLDÉ : {item.settledCount} · PARTIEL : {item.partialCount} · autres : {item.otherStatusCount} · rapprochées : {item.certifiedRowCount} ({item.certifiedPaidUsd} USD) · exclues : {item.excludedCount} ({item.excludedAmountUsd} USD)</p><p className="mt-2 text-xs text-amber-100">Raisons : {Object.entries(item.exclusionReasons).map(([reason, detail]) => `${reason} ${detail.count} lignes/${detail.amount} USD`).join(" · ") || "Aucune"}</p></div>)}</div></GlassPanel>
    </div> : null}
  </Container></main>;
}
