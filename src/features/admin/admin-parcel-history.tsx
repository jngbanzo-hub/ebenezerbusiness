...
export function AdminParcelHistoryView({ code }: { code: string }) {
  ...
  return <main className="min-h-screen bg-ebe-night py-8 text-white"><Container className="max-w-5xl">
    ...
    {history?.found?<div className="mt-7 space-y-6">
      ...
      {history.consistency==="MULTIPLE_MANIFEST_MATCHES"?<GlassPanel className="border-sky-300/30 p-6"><h3 className="font-bold text-sky-200">PLUSIEURS CORRESPONDANCES MANIFESTE TROUVÉES</h3><p className="mt-2 text-sm">Ces occurrences sont affichées séparément et ne constituent pas, à elles seules, une incohérence inter-sources.</p><p className="mt-2 text-sm">{history.manifestMatches.details.join(" · ")}</p></GlassPanel>:null}
      {history.consistency==="COHERENT"?<GlassPanel className="border-accent/30 p-4"><p className="font-bold text-accent">COHÉRENT</p></GlassPanel>:null}
