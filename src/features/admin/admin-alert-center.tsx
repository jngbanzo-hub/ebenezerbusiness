...
export function AdminAlertsPanel({onCount}:{onCount?:(count:number)=>void}){const [result,setResult]=useState<Result|null>(null),[error,setError]=useState("");const [level,setLevel]=useState<(typeof levels)[number]>("TOUTES"),[agency,setAgency]=useState<(typeof agencies)[number]>("TOUTES"),[category,setCategory]=useState<(typeof categories)[number]>("TOUTES");
useEffect(()=>{authenticatedRead(getSupabaseBrowserClient().auth,"/api/admin/alerts").then((response)=>readJsonOrThrow<Result>(response,"Centre d’alertes indisponible.")).then((value)=>{setResult(value);onCount?.(value.count)}).catch((cause)=>setError(cause instanceof Error?cause.message:"Centre d’alertes indisponible."));},[onCount]);
...
<section className="mt-6 space-y-4">{alerts.length===0?<GlassPanel className="p-8 text-center text-muted-foreground">Aucune alerte active pour ces filtres.</GlassPanel>:alerts.map((item)=><AlertCard key={item.id} item={item}/>)}</section></>:null}</>}
