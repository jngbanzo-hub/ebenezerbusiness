...
export function ShipmentTrackingPage() {
  ...
  const [filterOptions, setFilterOptions] = useState({ companies: [] as string[], destinations: [] as string[] });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(body.rows ?? []);
      setFilterOptions({ companies: body.options?.companies ?? [], destinations: body.options?.destinations ?? [] });
      setSelected(new Set());
  ...
  return <main className="min-h-screen bg-ebe-night py-8 text-white"><Container>
    ...
    <GlassPanel className="mt-8 p-5"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5"><Input label="Date de début" type="date" value={filters.from} onChange={(from)=>setFilters({...filters,from})}/><Input label="Date de fin" type="date" value={filters.to} onChange={(to)=>setFilters({...filters,to})}/><Select label="Compagnie" value={filters.company} options={["ALL",...filterOptions.companies]} onChange={(company)=>setFilters({...filters,company})}/><Select label="Destination" value={filters.destination} options={["ALL",...filterOptions.destinations]} onChange={(destination)=>setFilters({...filters,destination})}/><Select label="Statut" value={filters.status} options={["ALL",...SHIPMENT_STATUSES]} onChange={(status)=>setFilters({...filters,status})}/><Input label="Recherche groupage" value={filters.search} onChange={(search)=>setFilters({...filters,search})}/></div><div className="mt-4 flex flex-wrap gap-3"><Button variant="growth" disabled={!ready||loading} onClick={()=>void load()}>{loading?<LoaderCircle className="mr-2 h-4 w-4 animate-spin"/>:<RefreshCw className="mr-2 h-4 w-4"/>}Rafraîchir</Button><Button variant="outline" disabled={loading} onClick={resetFilters}>RÉINITIALISER LES FILTRES</Button></div></GlassPanel>
