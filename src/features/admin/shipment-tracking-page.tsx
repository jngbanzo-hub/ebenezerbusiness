...
export function ShipmentTrackingPage() {
  ...
  const rows = useMemo(() => filterShipmentTrackingRows(allRows, filters), [allRows, filters]);
  const filterOptions = useMemo(() => ({ companies: unique(allRows.map((row) => row.company)), destinations: unique(allRows.map((row) => row.destination)), statuses: unique(allRows.map((row) => row.status)) }), [allRows]);
