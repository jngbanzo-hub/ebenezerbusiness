...
async function request(url: string, init?: RequestInit) { const { data: { session } } = await getSupabaseBrowserClient().auth.getSession(); if (!session?.access_token) throw new Error("SESSION_REQUIRED"); const response = await fetch(url, { ...init, headers: { Authorization: `Bearer ${session.access_token}`, "Content-Type": "application/json", ...(init?.headers ?? {}) }, cache: "no-store" }); const value = await response.json(); if (!response.ok) throw new Error("NOTIFICATION_REQUEST_FAILED"); return value as Record<string, unknown>; }
