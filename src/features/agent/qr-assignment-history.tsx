...
export function QrAssignmentHistory({ refreshKey }: { refreshKey: number }) {
  ...
  return <section className="mt-6 rounded-xl border border-white/15 bg-white/5 p-4" aria-label="Historique des QR associés">
    <button type="button" aria-expanded={open} onClick={() => setOpen((value) => !value)} className="w-full rounded-lg border border-white/10 bg-white/5 px-4 py-3 text-left text-sm font-semibold transition hover:bg-white/10">
      {open ? "Masquer les associations récentes" : "Voir les associations récentes"}
    </button>
