...
test("forwarding is removed from Encaissements and moved to Stockage Sorties",()=>{assert.doesNotMatch(workspace,/ACHEMINER LE COLIS|handleForwarding|inter-agency-routing\/quote/);assert.match(stockages,/Acheminer un colis/);assert.match(stockages,/agency: "LSHI", rate: 13/);assert.match(stockages,/agency: "FIH", rate: 16/);assert.match(stockages,/data\.account\.agency === "KLZ" \|\| data\.account\.agency === "LSHI"/);});
test("legacy forwarding remittance is no longer rendered",()=>{assert.doesNotMatch(stockages,/Remise d’un acheminement|ForwardingCommandForm|forwardings\/delivery/);});
