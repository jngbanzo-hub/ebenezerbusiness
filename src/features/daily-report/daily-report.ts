...
export type DailyAgencyReport = Readonly<{
  ...
  storage: null | Readonly<{ openingParcels: number; openingWeightKg: number; arrivalsParcels: number; arrivalsWeightKg: number; departuresParcels: number; departuresWeightKg: number; closingParcels: number; closingWeightKg: number }>;
...
export function buildDailyAgencyReport(input: { agency: ReportAgency; payments: AdminPayment[]; expenses: AdminExpenseListResponse["depenses"]; storageEvents: StorageReportEvent[]; cash: DailyAgencyReport["cash"]; storage?: DailyAgencyReport["storage"]; adjustments?: DailyAgencyReport["adjustments"]; notes?: DailyAgencyReport["notes"] }): DailyAgencyReport {
  ...
  return Object.freeze({
    ...
    cash: input.cash,
    storage: input.agency === "COO" ? null : input.storage ?? null,
